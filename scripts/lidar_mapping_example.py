...
class LidarMapping():
    ...
    def laserscanCallback(self, msg):
        # 센서정보 전처리
        point_num = len(msg.ranges)
        angles = np.linspace(-np.pi, np.pi, point_num)
        distances = np.array(msg.ranges) # distances
        ...
        # main 함수 call
        map = self.main(angles, distances)

        map = map[0][:]
        map = map.T
        map = list(100*map.flatten())
        
        for index, value in enumerate(map):
            map[index] = int(value)
        #map = int(100*map.flatten())
        #print(map)

        # msg publishing
        map_msg = OccupancyGrid()
        #occupancy_grid = list(map.ravel())

        map_msg.header.frame_id = 'map'
        map_msg.info.resolution = 0.02
        map_msg.info.width = 350
        map_msg.info.height = 450
    ...
    def main(self, ang, dist):
        xy_resolution = 0.02  # x-y grid resolution

        ox = np.sin(ang) * dist
        oy = np.cos(ang) * dist
        occupancy_map, min_x, max_x, min_y, max_y, xy_resolution = \
            self.generate_ray_casting_grid_map(ox, oy, xy_resolution, True)
...
if __name__=="__main__":
    ...
    node_mapping = LidarMapping()
    
    try:
        node_mapping.spin_node()
